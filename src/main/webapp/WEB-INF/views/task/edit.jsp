	...
	<div class="d-flex justify-content-between">
		...
		<div id="admin-wrapper">
			...
			<section id="admin-content" class="p-3">
				<h3 class="mb-4">Chỉnh sửa task</h3>
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="text" name="id" class="form-control" readonly
									value="${ dto.id }" placeholder="email" />
							</div>
							<div class="form-group">
								<label>Name</label>
							...
							<div class="form-group">
								...
								<input type="date" name="startDate" class="form-control"
									value="${ dto.startDate }" placeholder="start date" />
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="date" name="endDate" class="form-control"
									value="${ dto.endDate }" placeholder="end date" />
							</div>
							<div class="form-group">
								...
								<select class="form-control" name="userId">
									<c:forEach items="${ userList }" var="item">
										<option value=${ item.id }
											${ dto.userId == item.id?'selected':'' }>${ item.fullname }
											(id: ${ item.id })</option>
									</c:forEach>
							...
							<div class="form-group">
								...
								<select class="form-control" name="projectId">
									<c:forEach items="${ projectList }" var="item">
										<option value=${ item.id }
											${ dto.projectId == item.id?'selected':'' }>${ item.name }(id:
											${ item.id })</option>
									</c:forEach>
							...
							<div class="form-group">
								...
								<select class="form-control" name="statusId">
									<c:forEach items="${ statusList }" var="item">
										<option value=${ item.id }
											${ dto.statusId == item.id?'selected':'' }>${ item.name }</option>
									</c:forEach>
